...
class LineGraphChart extends Component {
    constructor(props) {
      ...
      async componentDidMount(){
        ...
        const dates = this.props.orders.map(order => moment(order.createdAt).month() + 1)
        this.setState({ orders: this.props.orders, dates: dates },()=>{
            console.log('on mount orders', this.state.orders, 'on mount dates', this.state.dates);
          });
      }
      componentDidUpdate(prevProps){
        if(prevProps.orders.length !== this.props.orders.length){
            const dates = this.props.orders.map(order => moment(order.createdAt).month() + 1)
            this.setState({ orders: this.props.orders, dates: dates },()=>{
                console.log('on update', this.state.orders, 'on update', this.state.dates);
            });
          }
    }
    render() {
    const options =  {
        ...
        xaxis: {
          categories: ['Jan', 'Feb', 'March', 'April', 'May', 'June', 'July', 'Aug', 'Sept', 'October', 'Nov', 'December']
        }
    }
    const series = [
        {
